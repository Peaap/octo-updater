...
/// Compiles the vendored StormLib sources (pinned at tag v9.40, see
/// `vendor/stormlib`) directly into this crate via `cc`, mirroring the
/// upstream CMakeLists.txt file list for a Windows build with bundled
/// zlib/bzip2/LibTomCrypt/LibTomMath (the same configuration CMakeLists.txt
/// uses when `STORM_USE_BUNDLED_LIBRARIES` is on, which is what upstream
/// does by default on Windows since no system zlib/bzip2 is assumed).
///
/// Building from source (rather than depending on a prebuilt binary or a
/// separate `cmake` invocation) keeps `cargo build` self-contained and keeps
/// the exact StormLib revision reviewable and reproducible in this repo.
fn build_stormlib() {
    let root = Path::new("vendor/stormlib");
    let src = root.join("src");

    // `cc::Build` compiles every file it's given with one language setting, so
    // the C++ and C sources are built as two separate static libraries (both
    // linked into this crate) rather than trying to mix languages in one Build.
    ...
    if cfg!(windows) {
        // Matches CMakeLists.txt's WIN32 branch: bundle LibTomCrypt/LibTomMath
        // rather than requiring a system libtomcrypt, and link wininet (used by
        // StormLib's HTTP file-stream provider).
    ...
    let c_files = [
        ...
        // TOMCRYPT_FILES (only the subset StormLib actually references)
        ...
        // BZIP2_FILES (bundled: no system BZip2 assumed)
        ...
        // ZLIB_FILES (bundled: no system zlib assumed)
        "src/zlib/adler32.c",
        "src/zlib/compress.c",
        "src/zlib/crc32.c",
        "src/zlib/deflate.c",
    ...
    for relative_path in cpp_files {
        cpp_build.file(root.join(relative_path));
    }
    for relative_path in c_files {
        c_build.file(root.join(relative_path));
    }

    cpp_build.compile("stormlib_cpp");
    c_build.compile("stormlib_c");
    println!("cargo:rerun-if-changed=vendor/stormlib/src");
}
